...
class HttpClient:
    ...
    class ClientHandler(urllib.request.BaseHandler):
        ...
        def _response_decompress_content(self, request, response):
            ...
            if encoding == "gzip":
                fp = gzip.GzipFile(fileobj=StringIO(response.read()))
            elif encoding == "deflate":
                fp = zlib.decompress(response.read())
            else:
                fp = response.fp
        ...
        def http_response(self, request, response):
            self._response_extract_cookies(request, response)
            response = self._response_decompress_content(request, response)
            return response
